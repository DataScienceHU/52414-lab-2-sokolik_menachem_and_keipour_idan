...
For a symmetric (e.g. Normal) distribution: $־¼_3=0$.
Positive (negative) $־¼_3$ indicates long right (left) tail - therefor, theree is a long right tail.
...
```
The kurtosis is defined as: $־÷ג‰¡־¼_4גˆ’3$.
$\hat\mu_4 =\frac{\frac{1}{n}\sum _{i=1}^n\left(x_i-x\right)^4\:}{\left[\frac{1}{n}\sum \:_{i=1}^n\left(x_i-x\right)^2\:\right]^2}$
For the Normal distribution: $־÷=0$.
Positive (negative) $־÷$ indicates long (short) tails compared to Normal - therefor, there is a short left tail.
...
ggplot(data = subset(Diff_Overall_Potential,Group.1<=35),aes(x=Group.1,y=x))+
  geom_line() +geom_point() + labs(title="2.b. average Difference between a playerג€™s overall ability to potential ability by age") +
  xlab("Age")+
...
Find the 10 most-undervalued players.
...
knitr::kable(head(tmp[order(tmp$`ratio potential to value`,tmp$`median of potential`,decreasing = T),],10))
```
