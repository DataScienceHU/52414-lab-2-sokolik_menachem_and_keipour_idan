...
Positive (negative) $μ_3$ indicates long right (left) tail - therefor, theree is a long right tail.
...
Positive (negative) $κ$ indicates long (short) tails compared to Normal - therefor, there is a short left tail.
...
Since the average of its continent was greater than the overall average, then it can be said that the best continent is SOuth America, Africa.

Better seen by boxplot.
```{r}
# by boxplot:
 ggplot(data = fifa_players,aes(x=Continent,y = Overall))+
  geom_boxplot()+
  theme_light()+
  ylab("Overall")+
  coord_flip()+
...
b. Plot the average difference between a playerג€™s overall ability to potential ability as a function of age, up to age 35. At what ages should we expect to find players for future development based on this graph?

```{r}
average_Overall_Age <- aggregate(Overall, by = list(Age), FUN = mean)
P + geom_line(data= subset(average_Overall_Age, Group.1<=35), aes(x=Group.1,y=x), color ="red") + geom_point(data= subset(average_Overall_Age, Group.1<=35), aes(x=Group.1,y=x)) + geom_vline(xintercept = c(16,25), color = "#00Bfc4") + geom_text(x=20, y=70 , label="players for future \n development",size = 4, family="mono") + geom_text(x=33.5, y=75.5, label="red line is \n Overall average ",size = 3, family="mono", color = "red") +
...
ggplot(data = subset(Diff_Overall_Potential,Group.1<=35),aes(x=Group.1,y=x))+
  ...
  xlab("Age")+
  ylab("Potential vs Overall") + theme_light() + geom_vline(xintercept = c(16,25), color = "#00Bfc4") + geom_text(x=20, y=2, label="players for future \n development",size = 4, family="mono")
```
...
We expect to find players for future development between the ages of 16 and 25, this is because according to the graph there is a difference of 2.5 units (or more) the difference between the potential and the current skill.
...
potential <- subset(fifa_players, Overall>70 & Age<=21) %>% filter(Value !=0)
# Creating a linear model:
potential.fit <- lm(Value ~ Potential, data = potential)
# plotting wage and potential for these young players:
ggplot(data = potential, aes(y = Value, x=Potential)) + geom_point() +
  ggtitle(label = "c. Potential VS Value for players under 21") + 
...
potential.fit$coefficients
#knitr::kable(potential %>% arrange(desc(potential$predicted_Value/potential$Value)) %>% select(Name, Age, Potential, Overall, resid, Value, predicted_Value) %>% head(10))
knitr::kable(potential %>% arrange((resid)) %>% select(Name, Age, Potential, Overall, resid, Value, predicted_Value) %>% head(10), caption = "The ten most-undervalued players.")
```
...
fifa.3e <-  mutate(fifa.3e , ratio_med = Potential/(Value/1000))
nationality.ratio <- aggregate(ratio_med ~ Nationality, data = fifa.3e, FUN = median) %>% filter(ratio_med <1)
...
```{r}
tmp <- merge(nationality.ratio,nationality)
names(tmp)<-c("Nationality","ratio potential to value", "median of potential")
knitr::kable(head(tmp[order(tmp$`ratio potential to value`,tmp$`median of potential`,decreasing = T),],10), caption = "the ten under-valued country you'd recommend to travel.")
```

a country with cheap and high-potential players will result in a high value of the median ratio of potential to value of players. There is a certain trade-off, if I prefer a country with the highest ratio then I prefer to send my boss to San Marino. Otherwise, if I prefer a country with the median of the highest potential then I will send the my boss to Thailand, so the ratio is still somewhat high.
...
a. Calculate the correlation between players' ability in each pair of positions and show a heatmap correlation-plot of the correlations' matrix. What three positions have the *least* average correlations with other skills?
```{r}
...
We can see that the three positions which have the least average correlations with other skills are : "RCB","CB","LCB".
...
Can see that a goalkeeper's skills are on a steady rise which reaches its peak at 35 years old.
Many skills can be seen which begin to decline further as the skills of a delicate goalie rise.
Skills that peak at a young age is Balance, which at a young age is at its peak and over time consistently declines. Also, there are various skills that reach a peak at a slightly more advanced but young age for example Acceleration and agility.
...
b.
1. There were no titles to the plots. Itג€™s not obvious what the plots are trying to show.
2. The labels on the X-axis are overriding one another, and have to be rotated by 90גˆ˜ degrees or decrease the caption size.
...
5. The first plotג€™s Y-axis has a very non-informative also in others axis not so informative.
