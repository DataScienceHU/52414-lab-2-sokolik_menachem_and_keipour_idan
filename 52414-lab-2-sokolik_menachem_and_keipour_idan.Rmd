...
knitr::kable(potential %>% arrange(desc(resid)) %>% select(Name, Age, Potential, Overall, resid, Value, Market_Value) %>% head(10))
...
## Q3. Correlations Analysis
```{r}
position_corlation <- cor(fifa_players_positions[-c(1,28:30)])

corrplot(position_corlation,order = "hclust",tl.col = "black",tl.cex = 0.8,title = "position corrlation",cex.main = 1.5, cl.cex=0.8, pch.cex = 0.6,type = "lower", mar=c(0, 0, 1, 0))

attrib_corlation <-  cor(fifa_players_attribures[-c(1,13:17)])
corrplot(attrib_corlation,order = "hclust",tl.col = "black",tl.cex = 0.8,title = "ttributes corrlation",cex.main = 1.5, cl.cex=0.8, pch.cex = 0.6,type = "lower", mar=c(0, 0, 1, 0))
```
...
fifa_players_positions <- fifa[,c(1,46:72,6,7)] # players ability at different positions . Add overall 
fifa_players_indicators <- fifa[,c(1,6,7,10,11)] # players general ability 
fifa_players_positions_subset<-fifa_players_positions[,-c(13:17)]  # hard-coded solution: how were these positions selected?

```{r}
fifa_players_positions_subset = left_join(fifa_players_positions,fifa_players_info[,c(1,2)],"ID")
fifa_players_positions_subset <-fifa_players_positions_subset[!grepl("GK", fifa_players_positions_subset$Preferred.Positions),]

team <- function(df=fifa, positions){
  team_bast <- data.frame()
  for (i in positions){
    column <- which(names(df)== i)
    player.name <- df[which(df[, column] == max(df[,column])),]
...
vec_names <- c("CAM", "CB", "CM", "RB", "RW", "ST")

# In order to see the best players of each skill, we'll create a blank DF and insert sorted column for each skill:
team_options <- as.data.frame(matrix(ncol = 6, nrow = 5))
team_options_numbers <- as.data.frame(matrix(ncol = 6, nrow = 5))

# Running overa ll the skills, sorting the best players, and inserting the columns to the names and numbers data frames:
for(i in 1:6){
  current_name <- vec_names[i]
  current_col <- fifa_players[,current_name]
  temp_df <- data.frame(fifa_players$Name, current_col) %>% arrange(desc(current_col))
  team_options[,i] <- temp_df[1:5,1]
  team_options_numbers[,i]<- temp_df[1:5,2]
}

# changing names for the data frames columns:
colnames(team_options) <- vec_names
colnames(team_options_numbers) <- vec_names
