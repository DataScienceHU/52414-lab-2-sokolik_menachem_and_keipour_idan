...
skewness(players.Overall) # fat tails of players.Overall
```
...
kurtosis(players.Overall) # fat tails of players.Overall
```
...
head(arrange(ten.youngest, average_Age),10)
```


ten oldest  teams in terms of average players age:
```{r}
head(arrange(ten.youngest, desc(average_Age)),10)
```
...
a.
this function will be used on each club, in order to describe how scattered the club’s composition. The closer the number to $1$, the more it is described mainly by 1 dominant nationality. Since $p_i∈[0,1]$ we also know that $p^2_i∈[0,1]$ and the closer $p_i$ to 1, the closer $p^2_i$ to 1, so if there’s a nationality with a heavy weight, it’ll be shown as a big parameter DIV. The closer the DIV to 0, that means that each pi was closer to 0, meaning there was no concentration in one main nationality - which mean there’s diversity.

The purpose of the plots is to show of different nationalities in each club and brings the eight highest in terms of the variety of players. The second plot shows the 12 clubs with the lowest index-of-coincidence $∑K_{i=1}p^2_i$. The plot is lacking because it does not take into account the possibility that there are teams with a large number of players compared to other teams, and thus clubs with more players may appear to be more diverse because they have more nationality.
...
# Plot number of different nationalities in each club
g <- ggplot(data = number_of_nationality_in_club %>% head(8), aes(x = Club, y = N_nation,color = Club, fill = Club), cex =0.2)
g + geom_bar(stat="identity") + ylab("number of nationalities") + labs(title="c. Number of Nationalities for top 8 clubs") + theme(axis.text.x = element_text(face = "bold", size = 10, angle = 30, hjust = 0.9))+ theme(axis.text.x = element_text(face = "bold", size = 10, angle = 30, hjust = 0.9)) + geom_text(aes(y=N_nation + 1 , label = round(N_nation,2)), lwd = 3, show.legend = F)



# Plot DIV (diversity?) of different nationalities in each club
g <- ggplot(data = DIV_in_club %>% head(12),aes(x = Club,y = DIV
, color = Club,fill = Club), cex =0.2)
g <- g + geom_bar(stat="identity") + ylab("Index of Coincidence") + labs(title="c. Index of coincidence of Nationality for top 12 clubs") + theme(axis.text.x = element_text(face = "bold", size = 10, angle = 30, hjust = 0.9)) + geom_text(aes(y=DIV+0.01 , label = round(DIV,2)), lwd = 3, show.legend = F)
g
```
