...
52414: Lab 2 Solutions

## Q1. Basic Univariate Statistics.
Solution:
...
kable(skewness(players.Overall)) # fat tails of players.Overall
```
Can be measured with the third standardized moment (skewness).
$\hatμ_3=\frac{\frac{1}{n}\sum _{i=1}^n\left(x_i-x\right)^3\:}{\left[\frac{1}{n}\sum \:_{i=1}^n\left(x_i-x\right)^2\:\right]^{1.5}}$
For a symmetric (e.g. Normal) distribution: $μ_3=0$.
Positive (negative) $μ_3$ indicates long right (left) tail.

```{r}
zero.Overall <-  fifa_players$WaOverallge[fifa_players$Overall == 0]
kable(kurtosis(players.Overall)) # fat tails of players.Overall
```
...
Positive (negative) $κ$ indicates long (short) tails compared to Normal.
...
d. Most valued players:
```{r}
top_ten_Value <- fifa_players %>% select(Name,Overall, Value) %>% top_n(10, Value) %>% arrange(desc(Value))
kable(head(top_ten_Value,10)) # Top 10 by value
```

Most Overall players:
```{r}
top_ten_Overall <- fifa_players %>% select(Name, Overall, Value) %>% top_n(10, Overall) %>% arrange(desc(Overall))
kable(head(top_ten_Overall,10)) # Top 10 by Overall
```

The best player not in the top-10 valued players is: *Neymar*.

ten youngest teams in terms of average players age:
```{r}
ten.youngest <- aggregate(Age, by = list(Club), FUN=mean)
colnames(ten.youngest)[2] <- "average_Age"
kable(head(arrange(ten.youngest, average_Age),10))
```


ten oldest  teams in terms of average players age:
```{r}
...
ggplot(data = subset(Diff_Overall_Potential,Group.1<=35),aes(x=Group.1,y=x))+
  geom_line() +geom_point() + labs(title="2.b. average Difference between a player’s overall ability to potential ability by age") +
  xlab("Age")+
...
c. 
```{r}
potential <- subset(fifa_players, Overall>70 & Age<=21)

# Creating a linear model:
potential.fit <- lm(Potential ~ Value, data = potential)

# plotting wage and potential for these young players:
ggplot(data = potential, aes(x = Value, y=Potential)) + geom_point(aes(color=Overall)) +
...
potential <-  mutate(potential , Market_Value  = round((potential$Potential - potential.fit$coefficients[[1]]) / potential.fit$coefficients[[2]], 3))
# Find table with best residuals 
kable(potential %>% arrange(desc(resid)) %>% select(Name, Age, Potential, Overall, resid, Value, Market_Value) %>% head(10))
```
...
fifa <-  mutate(fifa , med = Potential)
nationality <- aggregate(med ~ Nationality, data = fifa, FUN = median)
...
# the number of players in each part of the UK will be of use during the next question:
Wales <- nationality$Ones[which(nationality$Nationality=="Wales")]
Ireland <-nationality$Ones[which(nationality$Nationality=="Republic of Ireland")]
Scotland <-nationality$Ones[which(nationality$Nationality=="Scotland")]
England <- nationality$Ones[which(nationality$Nationality=="England")]
...
mapCountryData(sPDF,nameColumnToPlot="med",colourPalette=colourPalette,catMethod='fixedWidth', mapTitle = "d. median of Potential by country")
```

e. Repeat the above analysis but this time display a world map where each country is colored by the median ratio of potential to value of players. Find an under-valued country you’d recommend to travel to (i.e. a country with cheap players compared to their potential average quality).
```{r}
#  add a column of the ratio of potential to value of players.
# fifa_players <-  mutate(fifa_players , ratio_med = Value/Potential)
# nationality.ratio <- aggregate(ratio_med ~ Nationality, data = fifa_players, FUN = median)
# 
# # median United kingdom we'll have to median the players in each part 
# ratio_med.UK <- median(nationality.ratio$med[which(nationality.ratio$Nationality=="England"|nationality.ratio$Nationality=="Wales"|nationality.ratio$Nationality=="Scotland"|nationality.ratio$Nationality=="Republic of Ireland")])
# 
# Wales <- nationality.ratio$ratio_med[which(nationality.ratio$Nationality=="Wales")]
# Ireland <-nationality.ratio$ratio_med[which(nationality.ratio$Nationality=="Republic of Ireland")]
# Scotland <-nationality.ratio$ratio_med[which(nationality.ratio$Nationality=="Scotland")]
# England <- nationality.ratio$ratio_med[which(nationality.ratio$Nationality=="England")]
# 
# 
# # add a row of united kingdom with the median.
...
# #ploting the world map:
# colourPalette <- brewer.pal(7,'RdYlGn') # Choosing a color
# mapCountryData(sPDF,nameColumnToPlot="ratio_med",colourPalette=colourPalette,catMethod='fixedWidth', mapTitle = "d. median ratio of potential to value of players by country")
```
...
The second plot shows the 12 clubs with the lowest index-of-coincidence $∑K_{i=1}p^2_i$. 
...
5. The first plot’s Y-axis has a very non-informative also in others axis not so informative.
